...
void main() {
  ...
  setUp(() {
    ...
    final loginViewModelInstance = TestableLoginViewProvider(userDetailRepo: mockRepo);
    GetIt.I.registerSingleton<LoginViewModel>(loginViewModelInstance);

    ChangeNotifierProvider.value(
      value: loginViewModelInstance,
    );
  ...
  testWidgets('SignUpScreen renders and has form fields', (WidgetTester tester) async {
    ...
    try {
            ...
            child: Builder(builder: (context) {
              return MultiProvider(
                providers: [
                  ...
                  ChangeNotifierProvider(
                      create: (_) => LoginViewModel(userDetailRepo: mockRepo)),
                ],
      ...
      final ctx = tester.element(loginViewFinder);
      // If you registered TestableLoginViewProvider in GetIt, fetch it from GetIt
      final loginModel = Provider.of<LoginViewModel>(ctx, listen: false);
      // Set the context, so mContext is not null when used
      loginModel.setContext(ctx);
      // final BuildContext ctx = tester.element(find.byType(LoginView));
      // // Set context for your provider instance
      // final loginModel = Provider.of<LoginViewModel>(ctx, listen: false);
      // loginModel.setContext(ctx);
