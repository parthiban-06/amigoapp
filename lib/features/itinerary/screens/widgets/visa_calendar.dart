...
import 'shared/calendar_utils.dart';
import 'visa_calendar_base.dart';
...
class VisaCalendar<T> extends StatefulWidget {
  ...
  final void Function(DateTime focusedDay)? onNextYearClicked;

  final void Function(DateTime focusedDay)? onPreviousYearClicked;
  ...
  /// bool value to show/hide calendar background color
  final bool showCalendarBgColor;

  /// Set Color for calendar background
  final Color calendarBgColor;

  /// Set calendar background radius
  final double calendarBgRadius;
  ...
  VisaCalendar({
    ...
    this.calendarFormat = CalendarFormat.month, //month
    this.availableCalendarFormats = const {
      CalendarFormat.month: 'Expand', //'Month'
      // CalendarFormat.twoWeeks: 'Collapse', //'2 weeks'
    ...
    this.onHeaderTapped,
    this.onNextYearClicked,
    this.onPreviousYearClicked,
    this.onHeaderLongPressed,
    this.onPageChanged,
    this.onFormatChanged,
    this.onCalendarCreated,
    this.showCalendarBgColor = true,
    this.calendarBgColor = Colors.grey,
    this.calendarBgRadius = 10.0,
  ...
  @override
  State<VisaCalendar<T>> createState() => _VisaCalendarState<T>();
}

class _VisaCalendarState<T> extends State<VisaCalendar<T>> {
  ...
  @override
  void didUpdateWidget(VisaCalendar<T> oldWidget) {
  ...
  bool get _shouldBlockOutsideDays =>
      ...
      widget.calendarFormat == CalendarFormat.month; // month
  ...
  Widget build(BuildContext context) {
    return Container(
      decoration: BoxDecoration(
        color: widget.showCalendarBgColor
            ? widget.calendarBgColor
            : Colors.transparent, // Background color
        borderRadius: BorderRadius.circular(
          widget.calendarBgRadius,
        ),
      ),
      child: Column(
        ...
        children: [
                ...
                return CalendarHeader(
                  ...
                  onLeftChevronTap: () {
                    if (widget.calendarFormat == CalendarFormat.week) {
                      Utils.announceMessage(S.of(context).previous_week);
                    } else {
                      Utils.announceMessage(S.of(context).previous_month);
                    }
                    _onLeftChevronTap();
                  },
                  onLeftYearChevronTap: () {
                    Utils.announceMessage(S.of(context).previous_year);
                    widget.onPreviousYearClicked?.call(value);
                  },
                  onRightYearChevronTap: () {
                    Utils.announceMessage(S.of(context).next_year);
                    widget.onNextYearClicked?.call(value);
                  },
                  onRightChevronTap: () {
                    if (widget.calendarFormat == CalendarFormat.week) {
                      Utils.announceMessage(S.of(context).next_week);
                    } else {
                      Utils.announceMessage(S.of(context).next_month);
                    }
                    _onRightChevronTap();
                  },
                  onHeaderTap: () {
          ...
          Flexible(
            ...
            child: Padding(
              padding: EdgeInsets.only(bottom: Sizes.sixteen),
              child: VisaCalendarBase(
                ...
                dowBuilder: (BuildContext context, DateTime day) {
                  ...
                  if (dowCell == null) {
                    final String weekdayString = widget
                        ...
                        _getFormattedWeekday(day, widget.locale!);
  ...
  String _getFormattedWeekday(DateTime day, String locale) {
    final langCode =
        locale.split('_').first; // handles cases like "es_ES", "en_US"

    final String fullWeekday = DateFormat.E(locale).format(day);

    if (langCode == 'en') {
      // English: return single character
      return fullWeekday[0];
    } else if (langCode == 'ar') {
      // Arabic: return up to 3 characters
      return fullWeekday.length >= 3
          ? fullWeekday.substring(0, 3)
          : fullWeekday;
    } else {
      // Other languages: return up to 2 characters
      return fullWeekday.length >= 2
          ? fullWeekday.substring(0, 2)
          : fullWeekday;
    }
  }
}
