...
class VisaCalendarBase extends StatefulWidget {
  ...
  VisaCalendarBase({
    ...
    this.availableCalendarFormats = const {
      ...
      CalendarFormat.twoWeeks: 'Collapse',
  ...
  @override
  State<VisaCalendarBase> createState() => _VisaCalendarBaseState();
}

class _VisaCalendarBaseState extends State<VisaCalendarBase> {
  ...
  @override
  void didUpdateWidget(VisaCalendarBase oldWidget) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        return SimpleGestureDetector(
          ...
          child: _buildCalendarContent(constraints),
        );
      },
    );
  }

  Widget _buildCalendarContent(BoxConstraints constraints) {
    return ValueListenableBuilder<double>(
      valueListenable: _pageHeight,
      builder: (context, value, child) {
        final height = _calculateHeight(constraints, value);
        return _buildAnimatedCalendar(height, child);
      },
      child: _buildCalendarCore(constraints),
    );
  }

  double _calculateHeight(BoxConstraints constraints, double value) {
    return constraints.hasBoundedHeight ? constraints.maxHeight : value;
  }

  Widget _buildAnimatedCalendar(double height, Widget? child) {
    return AnimatedSize(
  ...
  Widget _buildCalendarCore(BoxConstraints constraints) {
    return CalendarCore(
      ...
      onPageChanged: (index, focusedMonth) =>
          _handlePageChanged(index, focusedMonth, constraints),
  ...
  void _handlePageChanged(
      int index, DateTime focusedMonth, BoxConstraints constraints) {
    if (!_pageCallbackDisabled) {
      _updateFocusedDay(focusedMonth);
      _updatePageHeightIfNeeded(focusedMonth, constraints);
      _updatePreviousIndexAndNotify(index, focusedMonth);
    }
    _pageCallbackDisabled = false;
  }

  void _updateFocusedDay(DateTime focusedMonth) {
    if (!isSameDay(_focusedDay, focusedMonth)) {
      _focusedDay = focusedMonth;
    }
  }

  void _updatePageHeightIfNeeded(
      DateTime focusedMonth, BoxConstraints constraints) {
    if (_shouldUpdatePageHeight(constraints)) {
      final rowCount = _getRowCount(widget.calendarFormat, focusedMonth);
      _pageHeight.value = _getPageHeight(rowCount);
    }
  }

  bool _shouldUpdatePageHeight(BoxConstraints constraints) {
    return widget.calendarFormat == CalendarFormat.month &&
        !widget.sixWeekMonthsEnforced &&
        !constraints.hasBoundedHeight;
  }

  void _updatePreviousIndexAndNotify(int index, DateTime focusedMonth) {
  ...
  int _calculateFocusedPage(
    ...
    switch (format) {
      case CalendarFormat.month: //month
        return _getMonthCount(startDay, focusedDay);
      case CalendarFormat.twoWeeks: //twoWeeks
        return _getTwoWeekCount(startDay, focusedDay);
      case CalendarFormat.week: //week
